...
### with subcycles for ФАНЬ ЧЭНДА, ЛО ГУАНЬЧЖУН , ЦЗИНЬ ГУ ЦИГУАНЬ only
###################################################################
use strict;
use warnings;
use HTML::Parse;
...
my $in_file = './final/tmp.txt';
#my $in_file = './final/anthology2.html';
my $out_file = './final/tmp.sql';
my $out_file1 = './final/Inserted_poems_with_cycle_and_subcycle.txt';
...
#next if ($translator !~ /Галенович/);
			next if ($originals_block =~ /<p class="a2">Оригиналы/); #exclude originals
#next if ($originals_block =~ /Четыре времени/);
#next if ($originals_block =~ /Песни священных/); #Песни священных
